...
def main(**r):
    ...
    _test(docs, file=r['file'].split('.')[0].strip())
...
def cross_val(data=[], thres=[0.02, 0.05], folds=10,
...
def _test(data=[],file=''):
    ...
    for feature in issel:
        ...
        for is_shingle in isshingle:
            temp_feature[is_shingle] = cross_val(data=data, thres=thres,
    ...
    with open('dump/' + file + '_DE.pickle', 'wb') as handle:
